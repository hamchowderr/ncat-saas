...
Deno.serve(async (req) => {
  try {
    ...
    const body: VideoTrimRequest = await req.json()
    ...
    if (body.start !== undefined && !timeRegex.test(body.start)) {
      return new Response('start time must be in hh:mm:ss or mm:ss format', { status: 400 })
    }
    if (body.end !== undefined && !timeRegex.test(body.end)) {
      return new Response('end time must be in hh:mm:ss or mm:ss format', { status: 400 })
    }
    ...
    const ncatPayload = {
      ...
      id: body.id || `video-trim-${Date.now()}`
    }

    const ncatResponse = await fetch(`${NCAT_API_URL}/v1/video/trim`, {
      method: 'POST',
      headers: {
    ...
    if (ncatResult.job_id) {
      await supabase.from('jobs').insert({
        ...
        custom_id: body.id || `video-trim-${Date.now()}`,
