...
Deno.serve(async (req) => {
  try {
    ...
    if (body.length !== undefined && (typeof body.length !== 'number' || body.length < 1 || body.length > 60)) {
      return new Response('length must be between 1 and 60 seconds', { status: 400 })
    }

    if (body.frame_rate !== undefined && (typeof body.frame_rate !== 'number' || body.frame_rate < 15 || body.frame_rate > 60)) {
      return new Response('frame_rate must be between 15 and 60 fps', { status: 400 })
    }

    if (body.zoom_speed !== undefined && (typeof body.zoom_speed !== 'number' || body.zoom_speed < 0 || body.zoom_speed > 100)) {
